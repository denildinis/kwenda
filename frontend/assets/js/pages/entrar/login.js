;(() => {
  ...
  async function handleLogin(event) {
    ...
    try {
      const response = await fetch('/kwenda/backend/routes/authentication.php', {
        method: 'POST',
      ...
      setTimeout(() => {
        if (result.role === 'admin') {
          window.location.href = '/kwenda/frontend/pages/cadastrar-banco/index.php'
        } else {
          window.location.href = '/kwenda/frontend/pages/dashboard/index.php'
        }
      }, waitTime)
    } catch (error) {
      console.error(error)
